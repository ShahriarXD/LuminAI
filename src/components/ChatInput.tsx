...
function ActionButton({ icon: Icon, label, active, onClick }: { icon: React.ComponentType<{ className?: string }>; label: string; active?: boolean; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium transition-all duration-200 btn-press ${
        active ? "bg-accent/15 text-accent shadow-soft" : "text-muted-foreground hover:bg-muted hover:text-foreground"
      }`}
    >
